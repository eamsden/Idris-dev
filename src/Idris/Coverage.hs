...
genClauses fc n xs given
   = do i <- getIState
        ...
        logLvl 10 $ "Original: \n" ++
             showSep "\n" (map (\t -> showTm i (delab' i t True True False 0)) xs)
...
--         return (map (\t -> PClause n t [] PImpossible []) new)
  where getLHS i term
            | (f, args) <- unApply term = map (\t -> delab' i t True True False 0) args
            | otherwise = []

        lhsApp (PClause _ _ l _ _ _) = l
        lhsApp (PWith _ _ l _ _ _) = l

        noMatch i tm = all (\x -> case matchClause i (delab' i x True True False 0) tm of
                                          Right _ -> False
                                          Left miss -> True) xs
